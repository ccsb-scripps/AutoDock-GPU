...
__dpct_inline__ float positive_power(float a, uint exp)
{
...
SYCL_EXTERNAL __dpct_inline__ float fmod_pi2(float x)
{
...
SYCL_EXTERNAL __dpct_inline__ float fast_acos(float cosine)
{
	...
	float x4=x3*x;
        float ac =
            (((fast_acos_o * x4 + fast_acos_a) * x3 + fast_acos_b) * x2 +
             fast_acos_c) *
                x +
            fast_acos_d +
...
SYCL_EXTERNAL __dpct_inline__ sycl::float4 cross(sycl::float3 &u,
                                                 sycl::float3 &v)
{
        sycl::float4 result;
...
__dpct_inline__ sycl::float4 cross(sycl::float4 &u, sycl::float4 &v)
{
        sycl::float4 result;
        result.x() = u.y() * v.z() - v.y() * u.z();
        result.y() = v.x() * u.z() - u.x() * v.z();
...
SYCL_EXTERNAL __dpct_inline__ sycl::float4 quaternion_multiply(sycl::float4 a,
                                                               sycl::float4 b)
{
        sycl::float4 result = {
...
SYCL_EXTERNAL __dpct_inline__ sycl::float4 quaternion_rotate(sycl::float4 v,
                                                             sycl::float4 rot)
{
        sycl::float4 result;
...
SYCL_EXTERNAL void gpu_calc_energy(float *pGenotype, float &energy, int &run_id,
		...
		if ((rotation_list_element & RLIST_DUMMY_MASK) == 0) // If not dummy rotation
		{
			...
			if ((rotation_list_element & RLIST_GENROT_MASK) == 0) // If rotating around rotatable bond
			{
				...
				// is needed only if rotating around rotatable bond
                        ...
                        sycl::float4 quatrot_left = rotation_unitvec;
                        // Performing rotation
			...
			// Performing final movement and storing values
                        sycl::float4 qt = quaternion_rotate(atom_to_rotate, quatrot_left);
